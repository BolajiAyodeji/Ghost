import {definitionsService, type RequestContext} from '../../services/members-custom-fields';
...
const controller = {
    ...
    browse: {
        ...
        query(frame: Frame) {
            return definitionsService!.browse({filter: frame.options.filter as string | undefined});
        }
    },
    ...
    read: {
        ...
        query(frame: Frame) {
            return definitionsService!.read(frame.options.key);
        }
    },
    ...
    add: {
        ...
        query(frame: Frame) {
            return definitionsService!.add(requestContextFromFrame(frame), frame.data.members_custom_fields[0]);
        }
    },
    ...
    edit: {
        ...
        query(frame: Frame) {
            return definitionsService!.edit(requestContextFromFrame(frame), frame.options.key, frame.data.members_custom_fields[0]);
        }
    },
    ...
    destroy: {
        ...
        async query(frame: Frame) {
            await definitionsService!.destroy(requestContextFromFrame(frame), frame.options.key);
            return null;
        }
    }
};
